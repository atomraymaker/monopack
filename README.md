...
- `--mode test`: builds the runtime payload, copies only relevant tests for that pack, runs them inside the build target, and does not write a deploy zip.
- `--with-tests`: same test execution behavior as above, but in `deploy` mode so zip output is still produced if checks pass.
...
| `--mode deploy\|test` | `deploy` | `deploy` produces runtime payload + zip, `test` runs pack-scoped tests in build output (no zip). |
| `--with-tests` | `False` | Run pack-scoped tests as a gate in deploy mode. |
...
6. Run verifier and optional pack-scoped tests.
7. In deploy mode, write zip artifact and optional digest files.
